...
function relPath(base, filePath, to) {
    if (to) {
        to += '/';
    } else {
        to = '';
    ...
    if (filePath.indexOf(base) !== 0) {
        return to + filePath.replace(/\\/g, '/');
    }

    var newPath = filePath.substr(base.length).replace(/\\/g, '/');
    if (newPath[0] === '/') {
        return to + newPath.substr(1);
    }
...
var plugin = function(opts) {
    ...
    return through.obj(function(file, enc, cb) {
        ...
        transformFilename(file, opts.hash);
...
plugin.manifest = function(pth, opts) {
    ...
    return through.obj(function(file, enc, cb) {
        ...
        var revisionedFile = relPath(base, file.path, opts.toDir);
        var oldFile = relPath(file.base, file.path);

        var originalFile = path.join(path.dirname(oldFile), path.basename(file.revOrigPath)).replace(/\\/g, '/');

        if (opts.toDir) {
            originalFile = '/' + originalFile;
        }
        manifest[originalFile] = revisionedFile;
        if (lodash.endsWith(originalFile, '.tpl')) {
            var ori = originalFile.replace('.tpl', '').replace(/\//g, '.');
            var rev = revisionedFile.replace('.tpl', '').replace(/\//g, '.');
            manifest[ori] = rev;
        }
        cb();
    }, function(cb) {
