...
/**
 * juf 中工具类操作集合。{@link https://lodash.com/ lodash} 中所有方法都挂载在此名字空间下面。
 * @param  {String} path
 * @return {String}
 * @example
 *   /a/b//c\d/ -> /a/b/c/d
...
_.touch = function(path, mtime) {
    ...
    if (mtime instanceof Date) {
        ...
        juf.log.error('invalid argument [mtime]');
    }
    ...
    function normalize(pattern) {
        ...
        switch (type) {
            ...
            case '[object RegExp]':
                return pattern;
            default:
                juf.log.error('invalid regexp [%s].', pattern);
        }
    }
...
_.find = function(rPath, include, exclude, root) {
    ...
    if (path) {
        ...
        juf.log.error('unable to find [%s]: No such file or directory.', rPath);
    }
    return list.sort();
};
/**
...
_.del = function(rPath, include, exclude) {
    ...
    if (rPath && _.exists(rPath)) {
        ...
        if (/^(?:\w:)?\/$/.test(path)) {
            juf.log.error('unable to delete directory [%s].', rPath);
        }
        ...
        //juf.log.error('unable to delete [' + rPath + ']: No such file or directory.');
    }
    return removedAll;
};
...
_.copy = function(rSource, target, include, exclude, uncover, move) {
    ...
    if (source) {
        ...
        juf.log.error('unable to copy [%s]: No such file or directory.', rSource);
    }
    return removedAll;
};
...
_.upload = function(url, opt, data, content, subpath, callback) {
    if (typeof content === 'string') {
        ...
        juf.log.error('unable to upload content [%s]', (typeof content));
    }
    opt = opt || {};
    data = data || {};
    var endl = '\r\n';
    ...
    matches.forEach(function(item, index) {
        ...
        if (match !== item.negate) {
            ...
            keys.forEach(function(key) {
                ...
                var value = typeof properties[key] === 'object' ? juf.util.cloneDeep(properties[key]) : properties[key];
